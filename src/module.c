...
/* Similar to RM_ThreadSafeContextTryLock but this function
 * would block only for the specified amount of time if the server lock is already acquired.
 *
 * If successful (lock acquired) REDISMODULE_OK is returned,
 * otherwise REDISMODULE_ERR is returned and errno is set
 * accordingly. */
int RM_ThreadSafeContextTryLockFor(RedisModuleCtx *ctx, long long timeout_ns) {
  UNUSED(ctx);

  int res = moduleTryAcquireGILFor(timeout_ns);
  if(res != 0) {
      errno = res;
      return REDISMODULE_ERR;
  }
...
static inline void set_timeout(struct timespec *ts, long long timeout_ns) {
...
int moduleTryAcquireGILFor(long long timeout_ns) {
#ifdef __APPLE__
    /* macOS doesn't support pthread_mutex_timedlock, so we implement it
     * using pthread_mutex_trylock with a loop and nanosleep. */
    ...
    if (res != EBUSY) return res;  /* Error other than busy */

    struct timespec sleep_time, now;
    /* Sleep time: 1ms initially, will be adjusted if timeout is near */
    sleep_time.tv_sec = 0;
    ...
    while (1) {
        ...
        res = pthread_mutex_trylock(&moduleGIL);
        if (res == 0) return 0;  /* Success */
        if (res != EBUSY) return res;  /* Error other than busy */
        ...
        /* Adjust sleep time if we're close to timeout */
        long long remaining_ns = (timeout.tv_sec - now.tv_sec) * NANOSEC_PER_SECOND +
                                 (timeout.tv_nsec - now.tv_nsec);
        if (remaining_ns < sleep_time.tv_nsec) {
    ...
    return pthread_mutex_timedlock(&moduleGIL, &timeout);
#endif
}
